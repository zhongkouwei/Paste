...
- 加固剪贴板卡片渲染：图片预览 `src` 和卡片 `data-id` 现在会先经过 HTML 转义再写入模板属性。
- 根因：渲染进程只转义了文本预览内容，历史文件字段虽然通常由应用生成，但加载恢复路径会接受持久化数据，属性拼接也应遵守同一数据边界。
- 验证：`node --check src/main.js && node --check src/preload.js && node --check src/renderer.js`、`npm run build`。
- 影响范围：仅渲染进程卡片模板输出；剪贴板监听、历史持久化、搜索、复制和粘贴逻辑不变。
