...
- 优化图片历史较多时的搜索响应；根因是图片条目已经保存全分辨率 `dataURL` 到 `body`，渲染进程每次输入搜索词都会扫描完整 base64 内容，容易在大图片历史下造成卡顿。
- 现在搜索会跳过图片正文，只匹配图片标题和类型；文本、链接和代码条目仍保留正文全文搜索。
- 验证：`node --check src/main.js && node --check src/preload.js && node --check src/renderer.js`、`npm run build`。
- 影响范围：仅渲染进程搜索匹配逻辑；剪贴板采集、历史持久化、图片复制/粘贴和筛选逻辑不变。
